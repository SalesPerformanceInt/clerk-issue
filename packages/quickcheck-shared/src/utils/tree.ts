...
export type CreateDetachedNode<T extends DataObj> = (
  this: Tree<T> | void,
  dataObj: T,
) => TreeNode<T>;

const createDetachedNode = function <T extends DataObj>(
  this: Tree<T> | void,
  dataObj: T,
) {
  ...
  if (this?.getNodeById && node.parentId) {
    ...
    if (!parentNode)
      throw "Could not find parent node. Does not belong to tree";
...
export const getNodeInTreesById = <T extends DataObj>(
  trees: Tree<T>[],
  id: Id,
) => {
