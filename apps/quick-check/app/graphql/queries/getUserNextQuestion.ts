...
import { graphql, type GQLProxyUserData, type GraphQLClient, type User_Question_Bool_Exp } from "~/graphql"

import { getToday } from "~/utils/date"

export const GET_USER_NEXT_QUESTION = graphql(/* GraphQL */ `
  query GetUserNextQuestion($userId: uuid!, $today: date!, $where: user_question_bool_exp = {}) {
...
export async function getUserNextQuestion(
  ...
  const where: User_Question_Bool_Exp = currentQuestionId ? { id: { _neq: currentQuestionId } } : {}
