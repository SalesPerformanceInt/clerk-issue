...
const getPercentOverdue = ({ answerDate, performanceRating, lastAnsweredOn, latestReviewGap }: AnswerToReview) => {
  if (performanceRating === REVIEW_WRONG) return 1

  const daysSinceLastReview = lastAnsweredOn
    ? Math.floor((new Date(answerDate).getTime() - new Date(lastAnsweredOn).getTime()) / DAY_MS)
    : 0
...
const getDifficulty = (percentOverdue: number, { performanceRating, difficulty }: AnswerToReview) => {
...
const getReviewGap = (
  ...
  const baseDays = Math.round((difficultyWeight - REVIEW_DAYS_OVERFLOW) * percentOverdue)
  const updatedReviewGap = latestReviewGap * (REVIEW_DAYS_OVERFLOW + baseDays) || REVIEW_LATEST_REVIEW_GAP
...
export const reviewAnswer = (answerToReview: AnswerToReview) => {
  ...
  const { updatedDifficulty, difficultyWeight } = getDifficulty(percentOverdue, answerToReview)

  const updatedReviewGap = getReviewGap(percentOverdue, difficultyWeight, answerToReview)

  const reviewedAnswer: ReviewedAnswer = {
    difficulty: updatedDifficulty,
    latestReviewGap: updatedReviewGap,
    answerDate: answerToReview.answerDate,
    streak: answerToReview.performanceRating === REVIEW_WRONG ? 0 : answerToReview.streak + 1,
