...
export async function getQuestionItems(this: ContentStackSDKClient, query: (query: Query) => Query = (query) => query) {
  try {
    return await this.getAllEntries<QuestionItem>("questionitem", (q) => query(q.includeReference(["topic"])))
  } catch (error) {
