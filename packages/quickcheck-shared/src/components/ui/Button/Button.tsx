...
export interface ButtonProps
  extends Omit<React.ButtonHTMLAttributes<HTMLButtonElement> & ButtonVariantProps, "isDesktop" | "children"> {
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, background = "dark", variant = "primary", rightIcon, children, loading, disabled, ...props }, ref) => {
    const isDesktop = useIsDesktop()

    return (
      <button
        className={cn(buttonVariants({ background, variant, isDesktop, className }))}
        ref={ref}
      ...
      >
        ...
        <div className={twMerge("flex items-center justify-center", loading && "invisible")}>
          ...
          {rightIcon && (
            ...
            >
              <Icon icon={rightIcon} className="text-center text-base font-light leading-6" />
            </div>
          )}
        </div>
      </button>
