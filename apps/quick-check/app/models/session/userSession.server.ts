...
import { createSession, getSession, logout, type CreatedSession } from "./sessionUtils"
...
type UserSessionData = [string, User["user_id"] | undefined, User["tenant_id"] | undefined]

/**
 * User Session Helpers
 */

export async function getUserDataFromSession(request: Request): Promise<UserSessionData> {
...
export async function createUserSession({ request, remember, redirectTo, userId, tenantId, now, admin }: UserSession) {
  const session = await getSession(request)
