...
import { graphql, type GQLProxyAllData, type GraphQLClient, type User_Insert_Input } from "~/graphql"
  ...
  mutation UpsertUser($user: user_insert_input!, $tenantId: String!) {
    insert_user_one(
      ...
      on_conflict: {
        ...
        update_columns: [email, first_name, last_name, language_preference, phone_number, timezone, show_leaderboard]
      }
    ) {
      ...UserWithActiveToken
    }
...
export async function upsertUser(this: GraphQLClient, user: UpsertUserInput, proxyData: GQLProxyAllData) {
  ...
  try {
    ...
    if (isEmpty(data.insert_user_one?.active_tokens)) await generateNewToken.call(this, { userId, tenantId, now })
