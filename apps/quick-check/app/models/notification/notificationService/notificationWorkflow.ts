import type { NotificationWorkflowProps, Schedule, StartWorkflow } from "@salesperformanceint/notification-service-js"
...
type StartNotificationWorkflowProps = Omit<StartWorkflow<"notificationWorkflow">, "workflow" | "workflowId"> & {
...
export const startNotificationWorkflow = async ({
  ...
  const notificationHandle = await notificationClient.startWorkflow({
    ...
    workflowArgs,
    taskQueue: taskQueue ?? (QC_ENV === "development" ? "localNotification" : "accelerate_notification_service"),
  })
