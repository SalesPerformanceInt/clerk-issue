  ...
  query GetUserAchievements($userId: uuid!, $today: date!) {
    user_by_pk(user_id: $userId) {
      skills_attempted: user_questions_aggregate(
        ...
        where: { last_answered_on: { _is_null: false }, user_answers_aggregate: { count: { predicate: { _gt: 0 } } } }
      ) {
        aggregate {
          count
        }
          ...
          _or: [
            { expiration_date: { _is_null: false, _lt: $today } }
            {
              user_questions_aggregate: {
                count: { predicate: { _eq: 0 }, filter: { _or: [{ retired_on: { _is_null: true } }] } }
              }
            }
          ]
        }
      ...
      total_enrollments: user_enrollments_aggregate(
        where: { user_questions_aggregate: { count: { predicate: { _gt: 0 } } } }
      ) {
        aggregate {
          count
        }
      ...
      retired_questions: user_questions_aggregate(
        where: {
          ...
          user_answers_aggregate: { count: { filter: { correct: { _eq: true } }, predicate: { _gte: 2 } } }
        }
      ) {
        aggregate {
          count
...
export async function getUserAchievements(this: GraphQLClient, proxyData: GQLProxyUserData) {
  try {
    ...
    const completedEnrollments = achievements.completed_enrollments.aggregate?.count ?? 0
    const totalEnrollments = achievements.total_enrollments.aggregate?.count ?? 0
    const completedEnrollmentsPercentage = (completedEnrollments / totalEnrollments) * 100 || 0

    const retiredQuestions = achievements.retired_questions.aggregate?.count ?? 0
    const totalQuestions = achievements.total_questions.aggregate?.count ?? 0
    const retiredQuestionsPercentage = (retiredQuestions / totalQuestions) * 100 || 0
...
export type UserDashboardAchievements = NonNullable<Awaited<ReturnType<typeof getUserAchievements>>>
