...
export async function getUserWeeklyStreakCalendar(this: GraphQLClient, proxyData: GQLProxyUserData) {
  try {
    ...
    const result = await this.query({
      ...
      variables: {
        userId,
        monthAgo: DateTime.fromISO(now).minus({ month: 1 }).startOf("day").toISO()!,
      },
    ...
    const answerDates = user_by_pk.user_answers.map(({ created_at }) => DateTime.fromISO(created_at))
...
export type UserDashboardWeeklyStreakCalendar = NonNullable<Awaited<ReturnType<typeof getUserWeeklyStreakCalendar>>>
