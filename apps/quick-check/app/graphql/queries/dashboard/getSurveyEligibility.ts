...
export async function getSurveyEligibility(this: GraphQLClient, proxyData: GQLProxyUserData) {
  try {
    ...
    if (data.survey_dismissed && today < DateTime.fromISO(data.survey_dismissed).plus({ days: 2 }).toISODate()!)
      return false

    const eligible = data.user_enrollments.some(({ start_date, expiration_date, total, retired }) => {
      ...
      const { days: enrollmentDays } = expirationDate.diff(startDate, ["days"]).toObject()
      const { days: elapsedDays } = DateTime.fromISO(today).diff(startDate, ["days"]).toObject()
