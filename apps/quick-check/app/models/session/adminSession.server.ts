...
type AdminSession = Expand<CreatedSession & Pick<SessionData, SessionKeys.NOW | SessionKeys.ADMIN>>
...
export async function getAdminDataFromFromSession(request: Request): Promise<AdminSessionData> {
...
export async function createAdminSession({ request, remember, redirectTo, now, admin }: AdminSession) {
  const session = await getSession(request)
