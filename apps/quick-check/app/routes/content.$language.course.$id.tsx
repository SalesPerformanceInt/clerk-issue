...
export const loader = async ({ params, request }: LoaderFunctionArgs) => {
  try {
    ...
    const course = first((await contentStack.getCourses((query) => query.containedIn("uid", [id]))) ?? [])
    invariant(course, "Course not found")

    const quickcheckTaxonomy =
      isArray(course.metadata.quickcheck_taxonomy) && first(course.metadata.quickcheck_taxonomy)
        ...
        <div className="overflow-x-auto sm:-mx-6 desktop:-mx-8">
          <div className="inline-block min-w-full sm:px-6 desktop:px-8">
            <div className="mb-8 flex items-center justify-between px-4 pt-4 sm:p-0">
              <button className="flex items-center" onClick={() => navigate(`/content/${language}/courses`)}>
                <FontAwesomeIcon
                  icon={faChevronLeft}
              ...
              <h1 className="text-center text-4xl font-bold">{course.metadata.display_title}</h1>
              <div />
            </div>
                ...
                <tbody>
                  {questions.map((question, row) => (
                    <tr key={question.uid} className={`border-b ${row % 2 === 0 ? "bg-neutral-100" : "bg-white"}`}>
                      ...
                      <td className="whitespace-nowrap px-6 py-4">
                        {isArray(question.topic) ? first(question.topic)?.title : ""}
                      </td>
                    </tr>
                  ))}
                </tbody>
