...
export const EnrollmentSkillQuestionReviewSuspense: FC<
  EnrollmentSkillQuestionReviewSuspenseProps
> = ({ skillQuestionsPromise }) => {
  ...
  return (
    <Section className="mt-4" title={t("enrollment.skill.question_review")}>
      <Suspense
        fallback={
          <CardSkeleton className="flow h-36 w-full flex-grow p-6" title />
        }
      >
        <TypedAwait resolve={skillQuestionsPromise}>
          {(skillQuestions) =>
            skillQuestions ? (
              <EnrollmentSkillQuestionReview skillQuestions={skillQuestions} />
            ) : null
          }
        </TypedAwait>
      </Suspense>
...
const EnrollmentSkillQuestionReview: FC<EnrollmentSkillQuestionReviewProps> = ({
  skillQuestions,
}) => {
  return skillQuestions.map((question) => {
    ...
    const correctAnswers = question.user_answers.filter(
