...
const DaysOfWeek = () => (
  <div className="flex justify-between">
    {daysOfWeek.map((day) => (
      <p key={day.toFormat("cccc")} className="w-5 text-center text-xs font-semibold uppercase leading-4 text-text">
        {day.toFormat("ccccc")}
      </p>
    ))}
  </div>
...
const CalendarMonth: FC<CalendarMonthProps> = ({ calendar, now }, ref) => (
  <div className="flex-grow">
    ...
    {calendar.map((week) => (
      <CalendarWeek key={week.days[0]?.date.toISODate()} week={week} now={now} className="mt-4" />
    ))}
  </div>
)
