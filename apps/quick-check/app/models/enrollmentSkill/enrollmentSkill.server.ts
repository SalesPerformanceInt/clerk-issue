...
export const getEnrollmentSkillDashboard = async (
  ...
  const enrollmentSkillDashboardData =
    await userApolloClient.getEnrollmentSkillDashboardData(
      enrollmentId,
      skillId,
    );

  const enrollmentSkillDashboardQuestions =
    userApolloClient.getEnrollmentSkillQuestions(enrollmentId, skillId);
...
export type EnrollmentSkillDashboard = NonNullable<
  Awaited<ReturnType<typeof getEnrollmentSkillDashboard>>
>;
