...
export const notifyUserEnrollment = async ({ request, user, enrollment }: NotifyUserEnrollmentProps) => {
  ...
  if (firstQuestion?.active_on) {
    const schedule: Schedule =
      firstQuestion.active_on > today ? { scheduledDate: firstQuestion.active_on } : { now: true }
