  ...
  return (
    <ResponsiveContainer className="flex-1 items-center p-4" asChild>
      <section className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-center sm:gap-12">
        <FontAwesomeIcon icon={faBellSchool} className="text-9xl text-text-10" />
        ...
        <section className="flex flex-col gap-6 sm:gap-3">
          ...
          <p className="text-text-75 sm:text-lg">{t("user.dashboard.no_enrollments.description")}</p>

          {!isDesktop && <AccelerateButton background="light" tenantId={userDashboardData.tenant_id} />}
        </section>
      </section>
    </ResponsiveContainer>
  )
