...
import { faArrowsRotate, faChevronLeft, faTrash } from "@fortawesome/pro-solid-svg-icons"
...
import { getAdminApolloClientFromRequest, type EnrollUserEnrollment, type User_Answer } from "~/graphql"
...
import { useIsLoading, useMakeUserAction } from "~/components/UsersTable/components/UserRow"
...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  const courses = taxonTrees.map((tree) => treeNodeToRawNodeDatum(tree.rootNode))
...
export const action = async ({ request, params }: ActionFunctionArgs) => {
  try {
    ...
    if (user && userAction?.type === "UNENROLL" && userAction?.enrollmentId && userAction?.taxonomyId) {
      const enrollmentActionResponse = await handleUserEnrollment({
        request,
        user: { userId, tenantId: user.tenant_id },
        enrollmentNewData: {
...
const formatUserAnswer = (userAnswer?: Pick<User_Answer, "correct" | "created_at">) => {
        ...
        <div className="overflow-x-auto sm:-mx-6 desktop:-mx-8">
          <div className="inline-block min-w-full py-2 sm:px-6 desktop:px-8">
            <div className="mb-8 flex items-center justify-between px-4 pt-4 sm:p-0">
              <button className="flex items-center" onClick={() => navigate(`/admin/tenant/${user?.tenant_id}`)}>
                <FontAwesomeIcon
                  icon={faChevronLeft}
                  className="w-6 text-center text-4xl leading-6 text-primary-75 sm:w-[10px] sm:text-base"
                />
                <div className="ml-4 font-bold text-primary-75">{user?.tenant_id}</div>
              </button>
              ...
              <table className="min-w-full table-auto text-left text-sm">
                ...
                <tbody>
                  <tr className="bg-neutral-100">
                    <td className="whitespace-nowrap px-6 py-4">{formatDate(productSurvey?.created_at) || "---"}</td>
                    <td className="whitespace-nowrap px-6 py-4">{productSurvey?.sentiment}</td>
                    <td className="whitespace-nowrap px-6 py-4">{productSurvey?.comment}</td>
                ...
                <tbody>
                  {courses.map((course, row) => {
                    const enrollment = user?.user_enrollments.find(
                      ({ taxonomy_id }) => taxonomy_id === course.attributes?.uid,
                    )

                    if (!isAdminEnabled && !enrollment) return null

                    return (
                      <tr key={course.name} className={`border-b ${row % 2 === 0 ? "bg-neutral-100" : "bg-white"}`}>
                        <td className="whitespace-nowrap px-6 py-4">{course.name}</td>
                        <td className="whitespace-nowrap px-6 py-4">
                          {enrollment?.user_questions_aggregate.aggregate?.count ?? null}
                        </td>
                        <td className="whitespace-nowrap px-6 py-4">{formatDate(enrollment?.created_at)}</td>
                        <td className="whitespace-nowrap px-6 py-4">{formatDate(enrollment?.start_date)}</td>
                        <td className="whitespace-nowrap px-6 py-4">{formatDate(enrollment?.expiration_date)}</td>
                        ...
                        {isAdminEnabled && (
                          <td className="whitespace-nowrap px-6 py-4 text-center">
                            <Button
                              ...
                              loading={
                                state !== "idle" &&
                                parseUserActionRequest(formData)?.taxonomyId === course.attributes?.uid
                              }
                              onClick={() => {
                ...
                <tbody>
                  {user?.user_questions.map((question, row) => {
                    const course = courses.find(
                      (course) => question.user_enrollment.taxonomy_id === course.attributes?.uid,
                    )

                    return (
                      <tr key={question.id} className={`border-b ${row % 2 === 0 ? "bg-neutral-100" : "bg-white"}`}>
                        <td className="whitespace-nowrap px-6 py-4">{question.title || question.id}</td>
                        <td className="whitespace-nowrap px-6 py-4">{formatDate(question.active_on)}</td>
                        <td className="whitespace-nowrap px-6 py-4">{course?.name}</td>
                        <td className="whitespace-nowrap px-6 py-4">{formatUserAnswer(question.user_answers?.[0])}</td>
                        <td className="whitespace-nowrap px-6 py-4">{formatUserAnswer(question.user_answers?.[1])}</td>
                        <td className="whitespace-nowrap px-6 py-4">{formatUserAnswer(question.user_answers?.[2])}</td>
                        <td className="whitespace-nowrap px-6 py-4">
                          {question.retired_on && DateTime.fromISO(question.retired_on).toISODate()}
                        </td>
                      </tr>
                    )
                  })}
