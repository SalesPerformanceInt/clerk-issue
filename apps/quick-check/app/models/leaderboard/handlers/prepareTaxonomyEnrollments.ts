...
const filterGroupedEnrollmentsByUser = (userId: string) => (groupedEnrollments: EnrollmentsByTaxonomyId) => {
  return Object.entries(groupedEnrollments).filter(([_, enrollments]) =>
    enrollments.some((enrollment) => enrollment.user_id === userId),
  )
}

const mapEnrollmentsWithTaxonomy = async (mappedEnrollments: MappedEnrollmentsByTaxonomyId) => {
