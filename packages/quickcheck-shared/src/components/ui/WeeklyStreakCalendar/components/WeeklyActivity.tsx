...
const WeeklyActivity: FC<WeeklyActivityProps> = ({ calendar, now }) => {
  ...
  return (
    <div className="ml-2 flex flex-col items-center">
      <p className="text-center text-xs font-semibold uppercase leading-4 text-text">{t("common.weekly")}</p>
      <div className="relative flex flex-col items-center">
        ...
        {calendar.map(({ activity, days }) => {
          ...
          const future = sunday.startOf("day")! > DateTime.fromISO(now).startOf("day")

          return <CalendarStatus key={days[0]?.date.toISODate()} activity={activity} className="mt-4" future={future} />
        })}
      </div>
    </div>
  )
