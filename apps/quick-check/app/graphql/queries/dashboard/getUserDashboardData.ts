...
export async function getUserDashboardData(this: GraphQLClient, proxyData: GQLProxyUserData) {
  try {
    ...
    const activeEnrollments = dashboard_data.active_enrollments.aggregate?.count || 0
    const completedEnrollments = dashboard_data.completed_enrollments.aggregate?.count || 0
...
export type UserDashboardData = NonNullable<Awaited<ReturnType<typeof getUserDashboardData>>>
